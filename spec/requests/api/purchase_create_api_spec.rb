...
describe 'Transaction can be created on POST /api/v1/transactions'  do
    it 'successfully' do
        ...
        expect(parsed_body[:transaction][:product][:name]).to eq(product.name)
        expect(parsed_body[:transaction][:product][:token]).to eq(product.token)
        expect(parsed_body[:transaction][:type_of]).to eq('pix')
        expect(parsed_body[:transaction][:pix_setting][:token]).to eq(pix_setting.token)
        expect(parsed_body[:transaction][:customer_payment_method][:token]).to eq(customer_payment_method.token)
    end
